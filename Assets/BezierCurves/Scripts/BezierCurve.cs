...
public class BezierCurve : MonoBehaviour
{
    ...
    void OnDrawGizmos()
    {
        ...
        if (points.Length > 1)
        {
            ...
            for (int i = 0; i < points.Length - 1; i++)
            {
                var subPath = DrawCurve(points[i], points[i + 1], resolution);

                float distance = 0f;
                for (int j = 0; j < subPath.Count - 1; ++j)
                {
                    distance += Vector3.Distance(subPath[j], subPath[j + 1]);
                }
                ...
                allPoints.AddRange(subPath);
            }

            if (close)
            {
                var subPath = DrawCurve(points[points.Length - 1], points[0], resolution);
                float distance = 0f;
                for (int j = 0; j < subPath.Count - 1; ++j)
                {
                    distance += Vector3.Distance(subPath[j], subPath[j + 1]);
            ...
            var paths = DoEasing(allPoints, distances);
            Gizmos.color = Color.red;
            var size = new Vector3(1, 1, 1);
            size /= 2;
            paths.ForEach(x => Gizmos.DrawCube(x, size));
        }
    }

    List<Vector3> DoEasing(List<Vector3> path, List<float> distances)
    {
        ...
        var accelComp = points[0].GetComponent<Acceleration>();
        if (accelComp != null)
        {
            accel = accelComp.accel;
        }

        float distSum = 0f;
        while (pathIndex < path.Count - 1)
        {
            float distance = speed * dt + 1 / 2 * accel * dt * dt;
            if (distance < 0f)
            {
                distance = 0.01f;
            }
            ...
            while (sum < distance && pathIndex < path.Count - 1)
            {
                float diff = Vector3.Distance(path[pathIndex], path[pathIndex + 1]);
                sum += diff;
                distSum += diff;
                pathIndex++;

                if (distSum >= distances[distanceIndex])
                {
                    distSum -= distances[distanceIndex];
                    distanceIndex++;
                    accelComp = points[distanceIndex].GetComponent<Acceleration>();
                    if (accelComp)
                    {
                        accel = accelComp.accel;
                    }
    ...
    /// </param>
    public static List<Vector3> DrawCurve(BezierPoint p1, BezierPoint p2, int resolution)
    {
